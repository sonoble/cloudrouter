...
Source3: quagga-pimd-service-and-init
...
Patch1: quagga-pimd-service-and-init

%define __perl_requires %{SOURCE1}
%define tardir quagga

%description
************************************************************************
**** NetDEF / OpenSourceRouting untested Quagga package.
**** This is a build by our CI system (https://ci.netdef.org/) and might
**** be untested and broken. Use at your own risk. We do not recommend to
**** use this for any production.
************************************************************************
...
%package contrib
Summary: Contrib tools for %define tardir quagga
Group: System Environment/Daemons
...
%configure \
    ...
    --enable-gcc-rdynamic \
    --with-pkg-extra-version=-ci.NetDEF.org-%_VERSION_%-%_RELEASE_%
...
install -p -m 644 %{_builddir}/%{tardir}/redhat/zebra.service %{buildroot}%{_unitdir}/zebra.service
install -p -m 644 %{_builddir}/%{tardir}/redhat/isisd.service %{buildroot}%{_unitdir}/isisd.service
install -p -m 644 %{_builddir}/%{tardir}/redhat/ripd.service %{buildroot}%{_unitdir}/ripd.service
install -p -m 644 %{_builddir}/%{tardir}/redhat/ospfd.service %{buildroot}%{_unitdir}/ospfd.service
install -p -m 644 %{_builddir}/%{tardir}/redhat/bgpd.service %{buildroot}%{_unitdir}/bgpd.service
install -p -m 644 %{_builddir}/%{tardir}/redhat/babeld.service %{buildroot}%{_unitdir}/babeld.service
install -p -m 644 %{_builddir}/%{tardir}/redhat/ospf6d.service %{buildroot}%{_unitdir}/ospf6d.service
install -p -m 644 %{_builddir}/%{tardir}/redhat/ripngd.service %{buildroot}%{_unitdir}/ripngd.service
install -p -m 644 %{_builddir}/%{tardir}/redhat/pimd.service %{buildroot}%{_unitdir}/pimd.service

install -p -m 644 %{_builddir}/%{tardir}/redhat/quagga.sysconfig %{buildroot}/etc/sysconfig/quagga
install -p -m 644 %{_builddir}/%{tardir}/redhat/quagga.logrotate %{buildroot}/etc/logrotate.d/quagga
...
%changelog
* %_DATE_% %_USER_% %_EMAIL_% - %_VERSION_%.%_RELEASE_%
- Automatic build by NetDEF CI system

* Mon Mar 30 2015 Martin Winter <mwinter@netdef.org> - test-version
- Adapted to auto-build as part of the NetDEF / OpenSourceRouting CI system
- Enabled extra features: watchquagga, isis-topo-builder, fpm interface, better tracebacksa
